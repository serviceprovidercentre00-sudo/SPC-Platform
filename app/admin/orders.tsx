...
export default function AdminOrders() {
    ...
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        ...
        if (err.code === "permission-denied") {
          Alert.alert(
            "Permission Error",
            "Rules update karein ya login check karein.",
          );
        }
        setLoading(false);
      },
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      ...
      if (Platform.OS === "web")
        console.log(`Order ${id} marked as ${newStatus}`);
    } catch (error) {
  ...
  // Jab tak auth verification aur firestore stream dono ready na ho, tab tak loader dikhayein
  if (!isAuthVerified || loading)
    return (
        ...
        {orders.length === 0 ? (
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>
              Abhi koi naya order nahi aaya hai.
            </Text>
          </View>
        ) : (
          orders.map((item) => {
                ...
                <View style={styles.body}>
                  ...
                  <View style={styles.statusActions}>
                    <TouchableOpacity
                      style={[
                        styles.actionBtn,
                        currentStatus === "Pending" && styles.activeTabPending,
                      ]}
                    ...
                    >
                      <Text
                        style={[
                          styles.actionText,
                          currentStatus === "Pending" && { color: "#D4AF37" },
                        ]}
                      >
                        Wait
                      </Text>
                    ...
                    >
                      <Text
                        style={[
                          styles.actionText,
                          currentStatus === "Accepted" && { color: "#007BFF" },
                        ]}
                      >
                        Accept
                      </Text>
                    ...
                    >
                      <Text
                        style={[
                          styles.actionText,
                          currentStatus === "Completed" && { color: "#28A745" },
                        ]}
                      >
                        Done
                      </Text>
